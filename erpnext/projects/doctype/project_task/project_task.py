...
class Task(NestedSet):
    ...
    def validate_parent_expected_end_date(self):
        ...
        parent_exp_end_date = frappe.db.get_value(
            "Project Task", self.parent_task, "exp_end_date"
        )
    ...
    def validate_parent_project_dates(self):
        ...
        if project_end_date := frappe.db.get_value(
            "Project", self.project, "expected_end_date"
        ):
                ...
                if task_date and date_diff(project_end_date, getdate(task_date)) < 0:
                    frappe.throw(
                        _("{0}'s {1} cannot be after {2}'s Expected End Date.").format(
                            frappe.bold(
                                frappe.get_desk_link("Project Task", self.name)
                            ),
    ...
    def validate_status(self):
        ...
        if self.status != self.get_db_value("status") and self.status == "Completed":
            for d in self.depends_on:
                if frappe.db.get_value("Project Task", d.task, "status") not in (
                    "Completed",
                    "Cancelled",
                ):
                    frappe.throw(
    ...
    def validate_parent_template_task(self):
        if self.parent_task:
            if not frappe.db.get_value("Project Task", self.parent_task, "is_template"):
                parent_task_format = f"""<a href="#Form/Project-Task/{self.parent_task}">{self.parent_task}</a>"""
                frappe.throw(
    ...
    def validate_depends_on_tasks(self):
        if self.depends_on:
            for task in self.depends_on:
                if not frappe.db.get_value("Project Task", task.task, "is_template"):
                    dependent_task_format = (
                        f"""<a href="#Form/Project-Task/{task.task}">{task.task}</a>"""
                    )
                    frappe.throw(
    ...
    def check_recursion(self):
        ...
        for d in check_list:
            ...
            while len(task_list) > count:
                tasks = frappe.db.sql(
                    " select {} from `tabTask Depends On` where {} = {} ".format(
                        d[0], d[1], "%s"
                    ),
    ...
    def reschedule_dependent_tasks(self):
        ...
        if end_date:
            for task_name in frappe.db.sql(
                """
				select name from `tabProject Task` as parent
				where parent.project = %(project)s
					and parent.name in (
						select parent from `tabProject Task Depends On` as child
						where child.task = %(task)s and child.project = %(project)s)
			""",
                {"project": self.project, "task": self.name},
                as_dict=1,
            ):
                task = frappe.get_doc("Project Task", task_name.name)
                if (
                    task.exp_start_date
    ...
    def populate_depends_on(self):
        if self.parent_task:
            parent = frappe.get_doc("Project Task", self.parent_task)
...
def check_if_child_exists(name):
    child_tasks = frappe.get_all("Project Task", filters={"parent_task": name})
    child_tasks = [get_link_to_form("Project Task", task.name) for task in child_tasks]
    return child_tasks
    ...
    for task in tasks:
        if task.status == "Pending Review":
            if getdate(task.review_date) > getdate(today()):
                continue
        frappe.get_doc("Project Task", task.name).update_status()
...
def make_timesheet(source_name, target_doc=None, ignore_permissions=False):
    ...
    doclist = get_mapped_doc(
        "Project Task",
        source_name,
        {"Project Task": {"doctype": "Timesheet"}},
        target_doc,
...
def add_multiple_tasks(data, parent):
    ...
    new_doc["project"] = (
        frappe.db.get_value("Project Task", {"name": parent}, "project") or ""
    )
...
def on_doctype_update():
    frappe.db.add_index("Project Task", ["lft", "rgt"])
